...
ber=1e-5%目标误码率
snr_target = 10
SNR_target = 10^(snr_target/10);%误码率所需信噪比
pp=0.5
...
for j=1:length(SNR)
    ...
    for i=1:monte_carlo
        ...
        %计算Bob满足误码率所需功率
        Pt2(j)=(SNR_target)/((norm(fc'*HBC*pb))^2)
        if Pt1(j)>Pt2(j)
        ...
        if i==1
            ...
            sum_R11(j)=sum_R11(j)+e11;
            sum_R12(j)=sum_R12(j)+e12 ;
            sum_R13(j)=sum_R13(j)+e13 ;
...
hold on;
legend('E_{IN}','E_{IS}','E_{AD-OPC}');
xlabel('信噪比 (dB)');
ylabel('能量效率 (bit \cdot J^-^1 \cdot Hz^-^1)');
hold on;
%%\gamma_d=5